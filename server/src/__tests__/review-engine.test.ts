...
describe("analyzeDiff", () => {
  ...
  it("should call LLM adapter and return findings", async () => {
  ...
  it("should fail instead of returning no findings for partial JSON responses", async () => {
    ...
    const completeMock = vi.fn(async (_req: { maxTokens?: number }) => ({
      content: _req.maxTokens && _req.maxTokens > 4096 ? partialResponse : partialResponse,
      finishReason: "length",
