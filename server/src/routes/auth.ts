...
usersRouter.get("/", async (_req, res) => {
  try {
    ...
    logger.error("Failed to list users", { error: err instanceof Error ? err.message : String(err) });
...
usersRouter.put("/:id/password", async (req, res) => {
  try {
    ...
    logger.error("Failed to reset password", { error: err instanceof Error ? err.message : String(err) });
